...
- 同一張名片已經建立聯絡人後，再次從審核表單儲存需更新原 contact，不可新增第二筆。
...
此階段目標是讓 My Megi 從單人本地工具變成多人可使用的平台，先完成清楚、可驗證的登入、登出、使用者管理與資料隔離，不追求複雜企業級 IAM。

目前狀態：已完成基本登入/登出、bootstrap 系統管理員、使用者管理、三種角色、資料 owner 隔離、Web UI 導覽與受保護 API。原先規劃的 Logo 紀錄列表沒有實際工作流，已移除。

### 目標

使用者進入系統前必須登入。系統依角色決定可見功能與資料範圍：一般用戶只能看到自己的名片與聯絡人，內容管理員可以看到所有人的名片與聯絡人，系統管理員只能看到用戶管理。
...
- 系統管理員只能看到用戶管理，不可看到名片、聯絡人、OCR/LLM 結果。
...
原先規劃的 Logo 紀錄只有空資料表與空列表，沒有 Logo 上傳、切換或審計工作流。為避免用戶管理頁出現無效資訊，此功能已從 MVP 移除。
